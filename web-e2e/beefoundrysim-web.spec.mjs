...
async function configureApi(page) {
  await page.route('**/beefoundrysim-config.json', async (route) => {
    await route.fulfill({
...
test('Iris controller takes off and settles into hover', async ({ page }) => {
  ...
  await expect.poll(async () => {
    const state = JSON.parse(await page.evaluate(() => window.beefoundrysimEditor.getStateJson()));
    return state.simulationState?.time ?? 0;
  }, { timeout: 12_000 }).toBeGreaterThan(5.5);

  const runtime = JSON.parse(await page.evaluate(() => window.beefoundrysimEditor.getStateJson()));
...
test('obstacle scene loads the forklift and courier glTF visuals', async ({ page }, testInfo) => {
  ...
  const sceneState = JSON.parse(await page.evaluate(() => window.beefoundrysimEditor.getStateJson()));
...
test('obstacle-aware Iris controller initializes and runs in the browser', async ({ page }) => {
  ...
  const loaded = JSON.parse(await page.evaluate(() => window.beefoundrysimEditor.getStateJson()));
...
test('frontend recovers shared assets when the API starts after the page', async ({ page }) => {
  ...
  await page.goto('/');
  await expect.poll(() => page.evaluate(() => window.beefoundrysimEditorReady)).toBe(true);

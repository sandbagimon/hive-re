...
def _euler_from_wxyz(quaternion: tuple[float, float, float, float]) -> tuple[float, float, float]:
    w, x, y, z = quaternion
...
class IrisPayloadDeliveryController:
    ...
    def reset(self, observation: ControllerObservation) -> None:
        ...
        if IRIS_BODY_LINK not in observation.bodies:
            raise ValueError(f"Iris body observation is missing: {IRIS_BODY_LINK}")
        if PAYLOAD_BODY not in observation.bodies:
            raise ValueError(f"Payload body observation is missing: {PAYLOAD_BODY}")
        if ATTACHMENT_ID not in observation.attachments:
            raise ValueError(f"Payload attachment is missing: {ATTACHMENT_ID}")
        position = observation.bodies[IRIS_BODY_LINK].position
        self.phase = "spool"
    ...
    def _advance_mission(self, observation: ControllerObservation, attached: bool) -> None:
        body = observation.bodies[IRIS_BODY_LINK]
        ...
        segment_reached = elapsed >= self.segment_duration and position_error < 0.18 and speed < 0.3
        ...
        if self.phase == "spool" and elapsed >= self.segment_duration:
            ...
            self._start_segment("to_dropoff", (*DROPOFF, CRUISE_HEIGHT), 5.0, observation)
        elif self.phase == "to_dropoff" and segment_reached:
            self._start_segment("descend_dropoff", (*DROPOFF, HOOK_HEIGHT), 3.0, observation)
    ...
    def _trajectory_target(
        ...
        progress = _clamp((time - self.phase_started_at) / self.segment_duration, 0.0, 1.0)
        ...
        delta = tuple(
            self.segment_target[index] - self.segment_start[index] for index in range(3)
        )
        return (
            tuple(self.segment_start[index] + delta[index] * blend for index in range(3)),
...
def create_controller() -> IrisPayloadDeliveryController:
    return IrisPayloadDeliveryController()

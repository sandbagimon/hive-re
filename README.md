...
The robotics schema includes fixed-clock `joint_state` sensors with stable IDs, qpos/qvel payloads, simulation timestamps, and monotonic sequence numbers. It also defines link-mounted IMUs with an explicit local sensor pose: orientation is `world_from_sensor` xyzw, while angular velocity and proper linear acceleration are expressed in the sensor frame. MJCF export attaches an IMU site to the referenced link and emits MuJoCo frame-quaternion, gyro, and accelerometer channels. Sensor update rates are exact integer divisors of the physics rate and remain independent of UI refresh, pause gaps, and target real-time factor. Robot Tree sensor selection opens a live joint-state Inspector without changing the authoring scene.

...
The robotics schema includes fixed-clock `joint_state` sensors, link-mounted IMUs, and the first contact-sensor contract. Contact sensors target exactly one link or collider, use sum aggregation, and expose bounded world-frame points/normals plus normal force/impulse and tangent force. IMU orientation is `world_from_sensor` xyzw, while angular velocity and MuJoCo accelerometer output are expressed in the sensor frame. Sensor update rates are exact integer divisors of the physics rate and remain independent of UI refresh, pause gaps, and target real-time factor. Robot Tree selection opens a type-specific live Sensor Inspector; the real Qt workflow verifies joint-state and IMU sensors can coexist, pause, record, and reset without changing the authoring scene.

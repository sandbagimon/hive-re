...
> 增加 Simulation Speed / Real-Time Factor 控制：提供 0.25x、0.5x、1x、2x segmented control，
> 保持 MuJoCo timestep 不变，只调整每个 UI tick 的固定步预算；runtime state 发布 target/actual RTF，
> 轨迹和 recording 继续使用 simulation time，并以长时间运行测试验证不会漏步或重复采样。

...
- `beefoundrysim-config.json` 在运行时选择 HTTP API、WebSocket、API 版本、可选 project ID 与 token，前端不要求同源。
...
- Qt MainWindow 只加载 `BEEFOUNDRYSIM_FRONTEND_URL`/`--frontend-url`，没有 QWebChannel、project root、模拟 service 或本地前端文件。

...
SimLab 提供了一条可直接运行的 Iris 四旋翼 A→B 取送闭环。这里的起飞、姿态变化、货物增重、
...
机体下方。真空请求在最后下降阶段预启，但只有吸盘与箱体形成连续接触、锚点距离不超过
...
包裹按 36×28×22 cm、0.35 kg 的瓦楞纸箱建模，碰撞使用稳定的箱体几何，外观增加封箱胶带、
两条捆扎带、运单/条码、箱盖接缝和护角。视觉细节不额外制造薄片碰撞，避免胶带和标签导致
MuJoCo 接触抖动。

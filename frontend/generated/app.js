...
function renderAssets(assets) {
    ...
    const pendingLocalScenes = localSceneStatuses.filter((scene) => (scene.status !== 'ready'
...
configureViewport({
    ...
    resolveLocalSceneTexture: async (sceneId, textureId) => {
        const result = await bridge.call('getLocalSceneTexture', sceneId, textureId);
        if (result.ok && result.data)
            return result.data;
...
const lastLocalSceneStatuses = new Map();
async function loadAssetsWithRetry() {
    ...
    if (assets.ok && assets.data) {
        ...
        for (const localScene of assets.data.local_scenes) {
            ...
            if (localStatus === lastLocalSceneStatuses.get(localScene.scene_id))
                continue;

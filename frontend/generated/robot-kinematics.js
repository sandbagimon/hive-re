...
function rotateVector(quaternion, vector) {
    ...
    const result = multiplyQuaternions(multiplyQuaternions(rotation, [...vector, 0]), inverse);
    return result.slice(0, 3);
}
function compose(left, right) {
    const offset = rotateVector(left.quaternion, right.position);
    return {
        position: left.position.map((component, index) => component + offset[index]),
        quaternion: normalizeQuaternion(multiplyQuaternions(left.quaternion, right.quaternion)),
    };
}
function inverse(value) {
    const rotation = normalizeQuaternion(value.quaternion);
    const quaternion = [-rotation[0], -rotation[1], -rotation[2], rotation[3]];
    return {
        position: rotateVector(quaternion, value.position.map((component) => -component)),
        quaternion,
    };
}
...
function jointMotion(joint) {
    ...
    if (joint.type === 'prismatic') {
        return {
            position: axis.map((component) => component * joint.initial_position),
            quaternion: [0, 0, 0, 1],
        };
    }
...
export function jointLocalPose(joint) {

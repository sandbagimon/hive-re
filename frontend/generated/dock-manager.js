...
export class DockManager {
    ...
    drag = null;
    dragGhost = null;
    libraryTargetLeaf = null;
    constructor() {
        const find = (id) => {
            const value = document.getElementById(id);
            if (!value)
                throw new Error(`Missing dock element: #${id}`);
            return value;
        };
        this.container = find('dock-root');
        this.staging = find('panel-staging');
        this.library = find('panel-library');
        this.librarySearch = find('panel-library-search');
        this.libraryGroups = find('panel-library-groups');
        for (const root of this.staging.querySelectorAll('[data-panel]')) {
    ...
    openPanel(panel, options = {}) {
        ...
        if (!this.root)
            this.root = { type: 'leaf', id: 'root-leaf', panels: [], active: null };
        ...
        if (existing) {
            ...
            if (options.focus)
                this.root = setActive(this.root, existing.id, panel);
        }
        else {
    ...
    autoOpen(panel, relevant, options = {}) {
        if (!relevant || this.userClosed.has(panel))
            return;
        const wasOpen = this.isOpen(panel);
        this.openPanel(panel, options);
        if (!wasOpen && !options.focus)
            return;
    }
    ...
    restore() {
        try {
            const saved = JSON.parse(window.localStorage.getItem(DOCK_STORAGE_KEY) ?? 'null');
            if (saved?.root)
    ...
    render() {
        ...
        for (const meta of this.panels.values())
            this.staging.appendChild(meta.root);
    ...
    buildSplit(node) {
        ...
        node.children.forEach((child, index) => {
            if (index > 0)
                el.appendChild(this.buildSplitter(node, index - 1));
    ...
    buildSplitter(node, index) {
        ...
        splitter.addEventListener('pointerdown', (event) => {
            ...
            const children = [...parent.children].filter((item) => item.classList.contains('dock-child'));
            ...
            const onMove = (move) => {
                ...
                last = position;
                if (Math.abs(delta) < 0.0005 || !before || !after)
                    return;
                ...
                if (growBefore < 0.08 || growAfter < 0.08)
                    return;
    ...
    buildLeaf(node) {
        ...
        if (activeMeta)
            body.appendChild(activeMeta.root);
        else {
    ...
    bindEvents() {
        this.container.addEventListener('click', (event) => {
            ...
            const closePanelId = target.closest('[data-panel-close]')?.dataset.panelClose;
            if (closePanelId) {
                this.closePanel(closePanelId);
                return;
            }
            const addLeafId = target.closest('[data-leaf-add]')?.dataset.leafAdd;
            if (addLeafId) {
                this.openLibrary(addLeafId);
                return;
            }
            const tab = target.closest('.dock-tab');
            if (tab?.dataset.bottomTab) {
                this.activateTab(tab.dataset.leafId ?? '', tab.dataset.bottomTab);
            }
        });
        ...
        this.container.addEventListener('pointerdown', (event) => {
            const target = event.target;
            if (target.closest('[data-panel-close], .dock-leaf-add'))
                return;
            const tab = target.closest('.dock-tab');
            if (!tab?.dataset.bottomTab)
                return;
            const panel = tab.dataset.bottomTab;
            if (panel === 'viewport')
                return;
        ...
        this.library.addEventListener('click', (event) => {
            if (event.target === this.library.querySelector('.panel-library-scrim'))
                this.closeLibrary();
            const item = event.target.closest('[data-library-panel]');
            if (!item?.dataset.libraryPanel)
                return;
            const panel = item.dataset.libraryPanel;
            if (this.isOpen(panel)) {
                this.focusPanel(panel);
            }
    ...
    beginDrag(panel, x, y) {
        ...
        this.moveGhost(x, y);
        this.updateDrag(x, y);
        const onMove = (move) => this.updateDrag(move.clientX, move.clientY);
        const onUp = (up) => {
            window.removeEventListener('pointermove', onMove);
            window.removeEventListener('pointerup', onUp);
    ...
    updateDrag(x, y) {
        ...
        if (leafId === this.drag.targetLeaf && zone === this.drag.zone)
            return;
        this.clearDropOverlay();
        this.drag.targetLeaf = leafId;
        this.drag.zone = zone;
        if (body && zone)
            this.showDropOverlay(body, zone);
    }
    leafBodyAt(x, y) {
        for (const body of this.container.querySelectorAll('[data-leaf-body]')) {

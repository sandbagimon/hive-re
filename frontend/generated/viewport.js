...
import { applyProceduralSurface, createProceduralEnvironmentTexture, } from './procedural-materials.js';
...
const canvas = requiredElement('#viewport');
const toolbar = requiredElement('#viewport-toolbar');
...
void loadPhotographicEnvironment('./environments/polyhaven/abandoned_hopper_terminal_03_1k.hdr').then((texture) => {
...
scene.add(axes);
const editorFloor = new THREE.Mesh(new THREE.PlaneGeometry(60, 60), new THREE.MeshStandardMaterial({
    color: 0x23323c,
    roughness: 0.96,
    metalness: 0.02,
}));
...
scene.add(fillLight);
const editorSky = new THREE.Mesh(new THREE.SphereGeometry(1, 32, 16), new THREE.ShaderMaterial({
    side: THREE.BackSide,
    depthWrite: false,
    uniforms: {
...
scene.add(editorSky);
const citySky = new THREE.Mesh(new THREE.SphereGeometry(1, 32, 16), new THREE.ShaderMaterial({
    side: THREE.BackSide,
    depthWrite: false,
    uniforms: {
...
scene.add(citySky);
const cinematicSky = new THREE.Mesh(new THREE.SphereGeometry(1, 32, 16), new THREE.ShaderMaterial({
    side: THREE.BackSide,
    depthWrite: false,
    uniforms: {
...
const actorRenderSignatures = new Map();
const actorLoadRevisions = new Map();
const photorealModelStates = new Map();
const actorVisualAnimationStates = new Map();
const actorVisualAnimationLoadStates = new Map();
...
function updateEnvironmentAppearance(sceneData) {
    ...
    const environment = sceneData.simulation_config.visual_environment;
    ...
    renderer.setClearColor(cityEnvironmentVisible ? 0xb9d3e3
        : cinematicEnvironmentVisible ? 0x0b1520 : 0x101d29, 1);
    ...
    if (scene.fog instanceof THREE.Fog) {
        scene.fog.color.set(cityEnvironmentVisible ? 0xc4d8e2
            : cinematicEnvironmentVisible ? environment?.fog_color ?? 0x17232f : 0x243746);
    }
    ambient.color.set(cityEnvironmentVisible ? 0xcce5ff
        : cinematicEnvironmentVisible ? 0x6f91ad : 0xd9edff);
    ambient.groundColor.set(cityEnvironmentVisible ? 0x53665d
...
function geometryForActor(actor) {
    ...
    if (geomType === 'ellipsoid') {
        return new THREE.SphereGeometry(1, 40, 24).scale(size[0] ?? 0.5, size[1] ?? 0.5, size[2] ?? 0.5);
    }
...
function addShippingPackageDetails(mesh, actor) {
    if (actor.properties.visual_style !== 'shipping_package')
        return;
    ...
    const addBox = (dimensions, position, color, roughness = 0.65) => {
        const detail = new THREE.Mesh(new THREE.BoxGeometry(...dimensions), new THREE.MeshStandardMaterial({ color, roughness, metalness: 0 }));
    ...
    addBox([tapeWidth, halfY * 2 + 0.004, 0.003], [0, 0, halfZ + surfaceOffset], 0xd7b36a, 0.48);
    for (const side of [-1, 1]) {
        addBox([tapeWidth, 0.003, halfZ * 2], [0, side * (halfY + surfaceOffset), 0], 0xd7b36a, 0.48);
    }
    const strapColor = 0x263746;
    for (const offsetY of [-halfY * 0.52, halfY * 0.52]) {
        addBox([halfX * 2 + 0.004, 0.012, 0.004], [0, offsetY, halfZ + 0.002], strapColor, 0.58);
        for (const side of [-1, 1]) {
            addBox([0.003, 0.012, halfZ * 2], [side * (halfX + surfaceOffset), offsetY, 0], strapColor, 0.58);
        }
    }
    addBox([halfX * 0.66, halfY * 0.62, 0.0025], [halfX * 0.32, 0, halfZ + 0.004], 0xf0ede4, 0.9);
    for (let index = 0; index < 7; index += 1) {
        const width = index % 3 === 0 ? 0.004 : 0.002;
        addBox([width, halfY * 0.36, 0.001], [halfX * 0.12 + index * halfX * 0.045, 0, halfZ + 0.0055], 0x222222, 0.8);
    }
    const seamColor = 0x6f4528;
    addBox([0.002, halfY * 2, 0.002], [-halfX * 0.48, 0, halfZ + 0.003], seamColor, 0.9);
    for (const x of [-halfX, halfX]) {
        for (const y of [-halfY, halfY]) {
            addBox([0.018, 0.018, 0.018], [x * 0.96, y * 0.95, halfZ * 0.92], 0x8e5a32, 0.88);
        }
    }
}
function roundedRectangleGeometry(width, depth, height, radius) {
...
function addInsulatedDeliveryBagDetails(mesh, actor, loadRevision) {
    if (actor.properties.visual_style !== 'insulated_delivery_bag')
        return;
    const size = actor.properties.size ?? [0.18, 0.14, 0.11];
    ...
    const lid = new THREE.Mesh(roundedRectangleGeometry(width * 0.98, depth * 0.98, 0.032, 0.022), mesh.material);
    ...
    for (const y of [-halfY * 0.97, halfY * 0.97]) {
        addDetail(new THREE.BoxGeometry(width * 0.84, 0.009, 0.009), accentMaterial.clone(), [0, y, seamZ]);
        addDetail(new THREE.BoxGeometry(width * 0.88, 0.007, 0.008), zipperMaterial.clone(), [0, y * 1.012, seamZ + 0.011]);
    }
    for (const x of [-halfX * 0.97, halfX * 0.97]) {
        addDetail(new THREE.BoxGeometry(0.009, depth * 0.84, 0.009), accentMaterial.clone(), [x, 0, seamZ]);
        addDetail(new THREE.BoxGeometry(0.007, depth * 0.88, 0.008), zipperMaterial.clone(), [x * 1.012, 0, seamZ + 0.011]);
    }
    const verticalSeamGeometry = new THREE.CylinderGeometry(0.0045, 0.0045, bodyHeight * 0.72, 12).rotateX(Math.PI / 2);
    ...
    for (const y of [-halfY * 0.88, halfY * 0.88]) {
        ...
        addDetail(new THREE.TubeGeometry(handleCurve, 28, 0.007, 8, false), rubberMaterial.clone(), [0, 0, 0]);
    }
    const frontPanel = addDetail(roundedRectangleGeometry(width * 0.58, 0.008, bodyHeight * 0.42, 0.012), new THREE.MeshStandardMaterial({
        color: 0x22282d,
        roughness: 0.66,
        metalness: 0.02,
    }), [0, -halfY - 0.005, -0.005]);
    frontPanel.userData.deliveryBagPanel = true;
    const reflectiveStrip = addDetail(new THREE.BoxGeometry(width * 0.46, 0.004, 0.018), new THREE.MeshPhysicalMaterial({
        color: 0xc9d5d8,
        roughness: 0.22,
        metalness: 0.72,
        clearcoat: 0.55,
    ...
    const zipperPull = addDetail(new THREE.BoxGeometry(0.022, 0.012, 0.006), accentMaterial.clone(), [halfX * 0.52, -halfY - 0.012, seamZ + 0.016]);
    zipperPull.rotation.z = -0.28;
    for (const x of [-halfX * 0.72, halfX * 0.72]) {
        for (const y of [-halfY * 0.68, halfY * 0.68]) {
            addDetail(new THREE.CylinderGeometry(0.013, 0.015, 0.012, 16).rotateX(Math.PI / 2), rubberMaterial.clone(), [x, y, -halfZ - 0.002]);
        }
    }
    const textureUrl = new URL('./textures/delivery-bag-oxford-albedo.png', document.baseURI).href;
...
function operationsDeckTexture(repeatX, repeatY) {
...
function landingPadTexture(label, accent) {
...
function addOperationsGroundDetails(mesh, actor) {
    const size = actor.properties.size ?? [5, 5, 0.05];
    ...
    const markings = new THREE.Mesh(new THREE.PlaneGeometry(halfX * 2, halfY * 2), new THREE.MeshBasicMaterial({
        map: texture,
        transparent: true,
        depthWrite: false,
        toneMapped: false,
...
function addVisualDetail(root, geometry, material, position) {
    const detail = new THREE.Mesh(geometry, material);
    detail.position.set(...position);
...
function cinematicRoadMarkingTexture() {
...
function addCinematicWetAsphaltDetails(mesh, actor, loadRevision) {
    ...
    const textureUrl = new URL('./textures/cinematic-delivery/wet-asphalt-albedo.png', document.baseURI).href;
    ...
    const markings = addVisualDetail(mesh, new THREE.PlaneGeometry(halfX * 2, halfY * 2), new THREE.MeshBasicMaterial({
    ...
    for (const [index, puddle] of [
        ...
        const detail = addVisualDetail(mesh, new THREE.CircleGeometry(1, 40), index === 0 ? puddleMaterial : puddleMaterial.clone(), [x, y, halfZ + 0.004]);
...
function facadeSignTexture(title, subtitle, accent) {
    return canvasTexture(1024, (context, size) => {
...
function addRestaurantPickupDetails(mesh, actor) {
    ...
    addVisualDetail(mesh, new THREE.BoxGeometry(1.62, 0.018, 0.78), warmInterior, [-0.32, halfY + 0.012, -0.25]);
    ...
    const door = addVisualDetail(mesh, new THREE.BoxGeometry(0.55, 0.06, 1.12), frame.clone(), [1.02, halfY + 0.045, -0.22]);
    addVisualDetail(door, new THREE.BoxGeometry(0.42, 0.025, 0.82), glass.clone(), [0, 0.04, 0.05]);
    addVisualDetail(door, new THREE.SphereGeometry(0.035, 16, 10), new THREE.MeshStandardMaterial({ color: 0xd7b36c, metalness: 0.9, roughness: 0.18 }), [-0.18, 0.08, 0]);
    const sign = addVisualDetail(mesh, new THREE.PlaneGeometry(2.25, 0.56), new THREE.MeshBasicMaterial({
        map: facadeSignTexture('NIGHT KITCHEN', 'PICKUP • OPEN', '#ff7a3d'),
        toneMapped: false,
    }), [-0.12, halfY + 0.075, 0.72]);
    sign.rotation.x = -Math.PI / 2;
    const awningMaterials = [0xf0e3cc, 0xc8452f].map((color) => (new THREE.MeshPhysicalMaterial({ color, roughness: 0.65, sheen: 0.25 })));
    for (let index = 0; index < 9; index += 1) {
        const awning = addVisualDetail(mesh, new THREE.BoxGeometry(0.29, 0.5, 0.055), awningMaterials[index % 2].clone(), [-1.16 + index * 0.29, halfY + 0.25, 0.34]);
        awning.rotation.x = -0.12;
    }
    for (const x of [-halfX * 0.88, halfX * 0.88]) {
        const lamp = addVisualDetail(mesh, new THREE.SphereGeometry(0.07, 20, 12), new THREE.MeshStandardMaterial({
            color: 0xffd2a0,
            emissive: 0xff7a32,
...
function addResidentialDropoffDetails(mesh, actor) {
    ...
    for (const z of [-0.72, 0.18, 1.02]) {
        for (const x of [-0.82, 0, 0.82]) {
            const window = addVisualDetail(mesh, new THREE.BoxGeometry(0.47, 0.04, 0.52), warmWindow.clone(), [x, -halfY - 0.028, z]);
            ...
            const balcony = addVisualDetail(mesh, new THREE.BoxGeometry(0.68, 0.35, 0.055), trim.clone(), [x, -halfY - 0.16, z - 0.35]);
            balcony.castShadow = true;
        }
    }
    const entrance = addVisualDetail(mesh, new THREE.BoxGeometry(0.58, 0.06, 1.1), new THREE.MeshPhysicalMaterial({
        color: 0x172331,
        roughness: 0.2,
        metalness: 0.42,
        clearcoat: 0.65,
    }), [0, -halfY - 0.04, -1.05]);
    const address = addVisualDetail(entrance, new THREE.PlaneGeometry(0.3, 0.3), new THREE.MeshBasicMaterial({
        map: facadeSignTexture('B', 'DELIVERY', '#78e3a3'),
        toneMapped: false,
    }), [0, -0.04, 0.2]);
    address.rotation.x = Math.PI / 2;
    const canopy = addVisualDetail(mesh, new THREE.BoxGeometry(1.35, 0.64, 0.08), trim.clone(), [0, -halfY - 0.31, -0.42]);
    canopy.rotation.x = 0.06;
    const roofEdge = addVisualDetail(mesh, new THREE.BoxGeometry(halfX * 2.1, 0.08, 0.14), trim.clone(), [0, -halfY - 0.01, 1.67]);
    roofEdge.castShadow = true;
}
function addDynamicDeliveryVanDetails(mesh) {
    ...
    for (const x of [-0.46, 0.46]) {
        for (const y of [-0.37, 0.37]) {
            const wheel = addVisualDetail(mesh, new THREE.CylinderGeometry(0.17, 0.17, 0.12, 28), wheelMaterial.clone(), [x, y, -0.5]);
            addVisualDetail(wheel, new THREE.CylinderGeometry(0.075, 0.075, 0.125, 20), new THREE.MeshStandardMaterial({ color: 0xb6bec5, metalness: 0.9, roughness: 0.22 }), [0, 0, 0]);
        }
    }
    for (const [x, color] of [[0.735, 0xf5f3d2], [-0.735, 0xff4a35]]) {
        for (const y of [-0.23, 0.23]) {
            const lamp = addVisualDetail(mesh, new THREE.BoxGeometry(0.03, 0.12, 0.1), new THREE.MeshStandardMaterial({
                color,
                emissive: color,
    ...
    const sideSign = addVisualDetail(mesh, new THREE.PlaneGeometry(0.72, 0.32), new THREE.MeshBasicMaterial({
...
function addDynamicForkliftDetails(mesh) {
    ...
    const addBox = (size, position, material) => addVisualDetail(mesh, new THREE.BoxGeometry(...size), material, position);
    ...
    for (const x of [-0.66, 0.32]) {
        for (const y of [-0.54, 0.54]) {
            const wheel = addVisualDetail(mesh, new THREE.CylinderGeometry(x > 0 ? 0.31 : 0.25, x > 0 ? 0.31 : 0.25, 0.18, 28), rubber.clone(), [x, y, -0.68]);
            addVisualDetail(wheel, new THREE.CylinderGeometry(0.11, 0.11, 0.19, 20), new THREE.MeshStandardMaterial({ color: 0x596067, roughness: 0.28, metalness: 0.86 }), [0, 0, 0]);
        }
    }
    ...
    const beacon = addVisualDetail(mesh, new THREE.CylinderGeometry(0.055, 0.055, 0.09, 18), new THREE.MeshStandardMaterial({
        color: 0xffb12b,
        emissive: 0xff8a16,
...
function addDynamicCourierDetails(mesh) {
    mesh.material.transparent = true;
    mesh.material.opacity = 0;
    mesh.material.depthWrite = false;
    mesh.material.colorWrite = false;
    ...
    for (const x of [-0.25, 0.25]) {
        const wheel = addVisualDetail(mesh, new THREE.TorusGeometry(0.19, 0.025, 12, 32), rubber.clone(), [x, 0, -0.62]);
    ...
    addVisualDetail(mesh, new THREE.SphereGeometry(0.14, 24, 16), new THREE.MeshStandardMaterial({ color: 0xe7b18c, roughness: 0.72 }), [0.03, 0, 0.48]);
    addVisualDetail(mesh, new THREE.SphereGeometry(0.155, 24, 12, 0, Math.PI * 2, 0, Math.PI * 0.58), new THREE.MeshPhysicalMaterial({
        color: 0xffc438,
        roughness: 0.22,
        metalness: 0.24,
        clearcoat: 0.8,
    }), [0.03, 0, 0.55]);
    const bag = addVisualDetail(mesh, roundedRectangleGeometry(0.3, 0.24, 0.36, 0.035), new THREE.MeshStandardMaterial({ color: 0xe36a2e, roughness: 0.64 }), [-0.12, 0.11, 0.0]);
    bag.rotation.x = Math.PI / 2;
    const beacon = addVisualDetail(mesh, new THREE.SphereGeometry(0.035, 16, 10), new THREE.MeshStandardMaterial({
        color: 0x52d8ff,
        emissive: 0x25bce9,
...
function addPbrCourierAccessories(mesh) {
    const pack = addVisualDetail(mesh, roundedRectangleGeometry(0.34, 0.16, 0.48, 0.035), new THREE.MeshPhysicalMaterial({
        color: 0xe36a2e,
        roughness: 0.58,
        metalness: 0.04,
        clearcoat: 0.18,
    ...
    const logo = addVisualDetail(mesh, new THREE.PlaneGeometry(0.26, 0.12), new THREE.MeshBasicMaterial({
...
function addLandingPadDetails(mesh, actor, label, accent) {
    ...
    const decal = new THREE.Mesh(new THREE.PlaneGeometry(halfX * 1.86, halfY * 1.86), new THREE.MeshBasicMaterial({
        map: texture,
        transparent: true,
        depthWrite: false,
        toneMapped: false,
...
function addObstacleDetails(mesh, actor, accent, beacon = false) {
    ...
    for (const [index, z] of [-halfZ * 0.72, halfZ * 0.72].entries()) {
        const band = new THREE.Mesh(new THREE.BoxGeometry(halfX * 2 + 0.018, halfY * 2 + 0.018, 0.055), index === 0 ? bandMaterial : bandMaterial.clone());
        band.position.z = z;
    ...
    const cap = new THREE.Mesh(new THREE.BoxGeometry(halfX * 2 + 0.012, halfY * 2 + 0.012, 0.035), bandMaterial.clone());
    cap.position.z = halfZ + 0.017;
    cap.castShadow = true;
    cap.userData.actorDetail = true;
    mesh.add(cap);
    if (beacon) {
        const light = new THREE.Mesh(new THREE.SphereGeometry(Math.min(halfX, halfY) * 0.24, 20, 12), new THREE.MeshStandardMaterial({
            color: 0xffa8f5,
            emissive: accent,
...
function updatePhotorealModelDataset() {
...
function updateGltfAnimationDataset() {
...
function resetActorVisualAnimations() {
...
function updateActorVisualAnimationMotion(actorId, position, simulationTime) {
    const state = actorVisualAnimationStates.get(actorId);
    if (!state)
        return;
    ...
    else if (state.lastSimulationTime !== null
        && simulationTime < state.lastSimulationTime) {
        state.targetPlaybackRate = 0;
    }
...
function advanceActorVisualAnimations(deltaSeconds) {
    for (const state of actorVisualAnimationStates.values()) {
        state.playbackRate = THREE.MathUtils.damp(state.playbackRate, state.targetPlaybackRate, 10, deltaSeconds);
        state.motionBlend = THREE.MathUtils.damp(state.motionBlend, state.targetPlaybackRate > 0 ? 1 : 0, 12, deltaSeconds);
        if (state.targetPlaybackRate === 0 && state.playbackRate < 1e-3) {
            state.playbackRate = 0;
        }
        for (const instance of state.animation.instances) {
            instance.locomotionAction.setEffectiveTimeScale(state.playbackRate);
            instance.locomotionAction.setEffectiveWeight(instance.idleAction ? state.motionBlend : 1);
            if (instance.idleAction) {
...
function addConstructionFenceExtension(mesh, actor) {
    if (actor.properties.visual_style !== 'known_obstacle')
        return;
    ...
    const addSteel = (geometry, position) => {
    ...
    const postGeometry = new THREE.CylinderGeometry(0.027, 0.032, extensionHeight, 16).rotateX(Math.PI / 2);
    ...
    for (const z of [0.07, extensionHeight - 0.07]) {
        addSteel(new THREE.CylinderGeometry(0.018, 0.018, halfY * 1.8, 12), [0, 0, z]);
    }
    const wirePoints = [];
    const bottom = 0.1;
    ...
    for (let y = -span; y <= span; y += step) {
        wirePoints.push(new THREE.Vector3(0, y, bottom), new THREE.Vector3(0, Math.min(y + 0.55, span), top), new THREE.Vector3(0, y, top), new THREE.Vector3(0, Math.min(y + 0.55, span), bottom));
    }
    const wire = new THREE.LineSegments(new THREE.BufferGeometry().setFromPoints(wirePoints), new THREE.LineBasicMaterial({
        color: 0x9aa5aa,
        transparent: true,
        opacity: 0.68,
    }));
    ...
    mesh.add(wire);
    const warning = addSteel(new THREE.PlaneGeometry(0.22, 0.38), [-halfX - 0.012, 0, extensionHeight * 0.54]);
...
function addPhotorealActorVisual(mesh, actor, loadRevision) {
    ...
    void createFittedPbrVisual(config, renderer.capabilities.getMaxAnisotropy()).then((result) => {
        if (!actorLoadIsCurrent(actor.id, mesh, loadRevision)) {
            if (result.animation)
                stopFittedPbrAnimation(result.animation);
        ...
        if (result.animation)
            registerActorVisualAnimation(actor.id, result.animation);
...
function addActorVisualDetails(mesh, actor, loadRevision) {
    if (addPhotorealActorVisual(mesh, actor, loadRevision))
        return;
    const visualStyle = actor.properties.visual_style;
    if (visualStyle === 'shipping_package')
    ...
    else if (visualStyle === 'operations_ground')
        addOperationsGroundDetails(mesh, actor);
...
function visualTextureUrls(payload) {
    return [
...
function loadMaterialTexture(actorId, mesh, loadRevision, url, slot, colorTexture, scalar) {
    if (!url)
        return;
    new THREE.TextureLoader().load(url, (texture) => {
        ...
        if (slot === 'roughnessMap')
            mesh.material.roughness = scalar ?? 1;
        if (slot === 'metalnessMap')
            mesh.material.metalness = scalar ?? 1;
        mesh.material.needsUpdate = true;
    }, undefined, () => URL.revokeObjectURL(url));
}
function geometryFromBundle(payload) {
    const geometry = new THREE.BufferGeometry();
    ...
    geometry.boundingBox = new THREE.Box3(new THREE.Vector3(...payload.bounds.min), new THREE.Vector3(...payload.bounds.max));
    geometry.boundingSphere = new THREE.Sphere(new THREE.Vector3(...payload.bounds.sphere.slice(0, 3)), payload.bounds.sphere[3]);
    return geometry;
}
function resolveGeometryBundle(artifactId) {
    let promise = geometryBundleCache.get(artifactId);
    if (!promise) {
        promise = visualGeometryBundleResolver(artifactId).then((buffer) => (buffer ? decodeGeometryBundle(buffer) : null));
        geometryBundleCache.set(artifactId, promise);
        void promise.catch(() => geometryBundleCache.delete(artifactId));
    }
    return promise;
}
function loadLocalSceneTexture(sceneId, textureId, colorTexture, scale, offset, rotation) {
...
function addStreamedSceneActor(actor, sceneId, loadRevision) {
    ...
    void localSceneManifestResolver(sceneId).then(async (manifest) => {
        ...
        const texture = (textureId, colorTexture, scale, offset, rotation) => {
            const key = [
                textureId,
            ...
            if (!promise) {
                promise = loadLocalSceneTexture(sceneId, textureId, colorTexture, scale, offset, rotation);
                texturePromises.set(key, promise);
            }
            return promise;
        };
        const initializedMaterialTextures = new Set();
        const initializeMaterialTextures = (materialId) => {
            if (initializedMaterialTextures.has(materialId))
                return;
            ...
            for (const [textureId, slot, colorTexture] of assignments) {
                if (!textureId)
                    continue;
                void texture(textureId, colorTexture, definition.texture_scale, definition.texture_offset, definition.texture_rotation).then((loadedTexture) => {
                    if (!loadedTexture)
                        return;
...
function addRobotActor(actor, articulation, visualBundle, loadRevision) {
    const root = new THREE.Group();
    ...
    for (const link of articulation.links) {
        ...
        for (const visual of link.visual_geometries) {
            ...
            const mesh = new THREE.Mesh(geometryForRobotVisual(visual, Boolean(visualBundle)), new THREE.MeshStandardMaterial({
                color: new THREE.Color(rgba[0], rgba[1], rgba[2]),
    ...
    if (visualBundle && bundledMeshes.size > 0) {
        void resolveGeometryBundle(visualBundle).then((bundle) => {
            if (!bundle || !actorLoadIsCurrent(actor.id, root, loadRevision))
                return;
    ...
    for (const joint of articulation.joints) {
        ...
        if (joint.type === 'fixed' || joint.initial_position === 0)
            continue;
        ...
        if (joint.type === 'prismatic') {
            ...
            child.quaternion.multiply(new THREE.Quaternion().setFromAxisAngle(axis, joint.initial_position));
        }
    }
    return root;
}
...
function addColliderDebug(mesh, actor) {
    const wireframe = new THREE.LineSegments(new THREE.WireframeGeometry(mesh.geometry), new THREE.LineBasicMaterial({
    ...
    mesh.add(wireframe);
    const center = new THREE.Mesh(new THREE.SphereGeometry(0.055, 16, 10), new THREE.MeshBasicMaterial({ color: 0xffd166, depthTest: false }));
...
function nextActorLoadRevision(actorId) {
    const revision = (actorLoadRevisions.get(actorId) ?? 0) + 1;
    actorLoadRevisions.set(actorId, revision);
    return revision;
}
function actorLoadIsCurrent(actorId, object, revision) {
    return actorLoadRevisions.get(actorId) === revision && actorMeshes.get(actorId) === object;
}
function articulationForActor(actor, sceneData) {
    const articulationIds = actor.properties.articulation_ids;
    return sceneData.robotics?.articulations.find((item) => articulationIds?.includes(item.id)) ?? null;
}
function actorRenderSignature(actor, sceneData) {
...
function updateActorObject(object, actor) {
...
function updateShadowFrustum() {
    if (cityEnvironmentVisible || actorGroup.children.length === 0)
        return;
    ...
    keyLight.position.copy(focusSphere.center).addScaledVector(keyLightDirection, lightDistance);
...
function removeActorObject(actorId) {
...
function createActorObject(actor, sceneData) {
    const loadRevision = nextActorLoadRevision(actor.id);
    if (actor.type === 'robot') {
        const articulation = articulationForActor(actor, sceneData);
        if (!articulation)
            return null;
        const robot = addRobotActor(actor, articulation, articulation.visual_bundle ?? null, loadRevision);
    ...
    mesh.userData.actorId = actor.id;
    mesh.castShadow = !['operations_ground', 'cinematic_wet_asphalt'].includes(actor.properties.visual_style ?? '');
    mesh.receiveShadow = true;
    addColliderDebug(mesh, actor);
    ...
    if (cachePath) {
        void visualGeometryResolver(cachePath).then((payload) => {
            if (!payload)
                return;
            if (!actorLoadIsCurrent(actor.id, mesh, loadRevision)) {
                for (const url of visualTextureUrls(payload))
                    URL.revokeObjectURL(url);
                return;
            }
            mesh.geometry.dispose();
            ...
            if (typeof payload.roughness === 'number')
                mesh.material.roughness = payload.roughness;
            if (typeof payload.metalness === 'number')
                mesh.material.metalness = payload.metalness;
            mesh.material.needsUpdate = true;
            loadMaterialTexture(actor.id, mesh, loadRevision, payload.base_color_texture_url, 'map', true);
            loadMaterialTexture(actor.id, mesh, loadRevision, payload.normal_texture_url, 'normalMap', false);
            loadMaterialTexture(actor.id, mesh, loadRevision, payload.roughness_texture_url, 'roughnessMap', false, payload.roughness);
            loadMaterialTexture(actor.id, mesh, loadRevision, payload.metallic_texture_url, 'metalnessMap', false, payload.metalness);
...
function syncAttachmentVisuals(sceneData) {
    ...
    for (const attachment of attachments) {
        ...
        if (attachment.gripper) {
            ...
            const plate = new THREE.Mesh(new THREE.BoxGeometry(plateX * 2, plateY * 2, plateZ * 2), metal);
            plate.position.z = gripper.cup_height + plateZ;
            parentMarker.add(plate);
            const mount = new THREE.Mesh(new THREE.CylinderGeometry(gripper.mount_radius, gripper.mount_radius, gripper.mount_length, 20).rotateX(Math.PI / 2), metal.clone());
            ...
            for (const [offsetX, offsetY] of [
                ...
                const cup = new THREE.Mesh(new THREE.CylinderGeometry(gripper.cup_radius * 0.72, gripper.cup_radius, gripper.cup_height, 24).rotateX(Math.PI / 2), rubber.clone());
                cup.position.set(offsetX, offsetY, gripper.cup_height * 0.5);
                parentMarker.add(cup);
                const lip = new THREE.Mesh(new THREE.TorusGeometry(gripper.cup_radius * 0.82, 0.0025, 8, 24), rubber.clone());
                lip.position.set(offsetX, offsetY, 0.001);
                parentMarker.add(lip);
            }
            indicator = new THREE.Mesh(new THREE.SphereGeometry(0.008, 14, 10), new THREE.MeshStandardMaterial({
                color: 0xf4b942,
                emissive: 0x5a3500,
            ...
            parentMarker = new THREE.Mesh(new THREE.SphereGeometry(Math.max(attachment.contact_probe_radius, 0.015), 12, 8), new THREE.MeshBasicMaterial({ color: 0xf4b942 }));
            indicator = parentMarker;
        }
        const childMarker = new THREE.Mesh(new THREE.SphereGeometry(0.015, 12, 8), new THREE.MeshBasicMaterial({ color: 0x4ed38a }));
...
function attachmentBodyObject(bodyId) {
    return robotLinkGroups.get(bodyId) ?? actorMeshes.get(bodyId) ?? null;
}
function updateAttachmentVisuals() {
    const runtimeStates = new Map((simulationState?.attachments ?? []).map((item) => [item.id, item]));
    ...
    for (const attachment of currentScene.attachments ?? []) {
        ...
        visual.indicator.material.color.set(connected ? 0x4ed38a : requested ? 0xf4b942 : 0x75808a);
        if (visual.indicator.material.emissive) {
            visual.indicator.material.emissive.set(connected ? 0x0c5c35 : requested ? 0x5a3500 : 0x111820);
        }
...
function syncNavigationVisual(sceneData) {
    ...
    for (let index = 0; index < routePoints.length - 1; index += 1) {
        ...
        if (length < 1e-6)
            continue;
        const ribbon = new THREE.Mesh(new THREE.CylinderGeometry(0.018, 0.018, length, 8, 1, true), new THREE.MeshBasicMaterial({
            color,
            transparent: true,
            opacity: 0.42,
            depthTest: false,
    ...
    for (const [index, point] of route.entries()) {
        const marker = new THREE.Mesh(new THREE.TorusGeometry(index === route.length - 1 ? 0.09 : 0.064, 0.012, 8, 24), new THREE.MeshBasicMaterial({
            color,
            transparent: true,
            opacity: 0.92,
            depthTest: false,
...
export function setViewportScene(sceneData) {
    ...
    const renderableActors = currentScene.actors.filter((actor) => actor.type === 'object' || actor.type === 'robot');
    ...
    for (const actor of renderableActors) {
        ...
        if (!object)
            object = createActorObject(actor, currentScene);
        if (!object)
            continue;
    ...
    const addedActors = currentScene.actors.filter((actor) => {
        if (!actorMeshes.has(actor.id) || previousActorIds.has(actor.id))
            return false;
...
export function selectViewportActor(actorId, notify = false) {
    ...
    updateHud();
    if (notify)
        actorSelectedCallback(actorId);
}
export function selectViewportLink(linkId) {
    const group = linkId ? robotLinkGroups.get(linkId) : null;
    selectedLinkId = group ? linkId : null;
    if (group?.userData.actorId)
        selectedActorId = group.userData.actorId;
    transformControls.detach();
    updateSelectionMaterials();
    updateSelectionOutline();
    updateHud();
...
let disabledFog = null;
export function setViewportFeature(feature, enabled) {
    if (feature === 'grid') {
...
function updateSelectionMaterials() {
    for (const [id, object] of actorMeshes.entries()) {
        ...
        object.traverse((mesh) => {
            ...
            mesh.material.emissive = new THREE.Color(linkSelected ? 0x9a6a16 : fillHighlight ? 0x2b6cb0 : 0);
...
function updateHud() {
    ...
    const navigationText = navigation && navigation.status !== 'idle'
        ...
        : '';
    const activeEvent = simulationState?.dynamic_events?.find((event) => event.status === 'active');
...
export function applySimulationState(state) {
    ...
    transformControls.detach();
    const animatedActorsInFrame = new Set();
    ...
    for (const [actorId, animation] of actorVisualAnimationStates.entries()) {
        if (!animatedActorsInFrame.has(actorId))
            animation.targetPlaybackRate = 0;
    }
    updateGltfAnimationDataset();
...
function setTransformMode(mode) {
    ...
    const selectedMesh = selectedActorId ? actorMeshes.get(selectedActorId) : null;
    if (selectedMesh && !selectedLinkId && !simulationState)
        transformControls.attach(selectedMesh);
}
function setColliderDebugVisible(visible) {
    ...
    const button = toolbar.querySelector('[data-action="collider-debug"]');
    button?.classList.toggle('active', visible);
    button?.setAttribute('aria-pressed', String(visible));
    const legend = requiredElement('#collider-legend');
    legend.hidden = !visible;
    updateHud();
}
function updateColliderDebugMarkers() {
    for (const mesh of actorMeshes.values()) {
        mesh.traverse((child) => {
...
function setFocusBounds(object) {
    ...
    for (const child of actorGroup.children) {
        const actor = child.userData.actor;
        if (actor?.properties.visual_style === 'operations_ground')
            continue;
...
toolbar.addEventListener('click', (event) => {
    ...
    else if (button.dataset.action === 'frame')
        frameObject(getFocusObject());
    else if (button.dataset.action === 'collider-debug')
        setColliderDebugVisible(!colliderDebugVisible);
...
function animate(frameTime = performance.now()) {
    ...
    if (frameTime - lastViewportFrame < viewportFrameInterval)
        return;

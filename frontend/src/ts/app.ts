...
function articulationsForActor(actor: Actor, scene: Scene) {
  const articulationIds = actor.properties.articulation_ids as string[] | undefined;
  return scene.robotics?.articulations.filter(
...
function renderSceneTree(
  ...
  for (const button of tree.querySelectorAll<HTMLButtonElement>('[data-sensor-id]')) {
    button.addEventListener('click', () => {
      const actorId = button.dataset.ownerActorId;
      const sensorId = button.dataset.sensorId;
      if (actorId && sensorId) store.selectSensor(actorId, sensorId);
    });
  }
}
...
function renderProperties(
  ...
  const selectedSensorSample = simulationState?.sensors.find(
  ...
  const sensorColliderLink = selectedSensor?.collider_id
    ? sensorArticulation?.links.find((link) =>
      link.colliders.some((collider) => collider.id === selectedSensor.collider_id))
    : undefined;
  const sensorCollider = sensorColliderLink?.colliders.find(
...
function renderArmControl(
  ...
  for (const input of body.querySelectorAll<HTMLInputElement>('[data-joint-target]')) {
    input.addEventListener('change', () => {
      const jointId = input.dataset.jointTarget;
      if (jointId) void sendJointTargets({ [jointId]: Number(input.value) });
    });
  }
  body.querySelector<HTMLInputElement>('[data-joint-step]')?.addEventListener('change', (event) => {
  ...
  for (const button of body.querySelectorAll<HTMLButtonElement>('[data-joint-jog]')) {
    button.addEventListener('click', () => {
      const jointId = button.dataset.jointJog;
      const direction = Number(button.dataset.direction);
      const step = Number(body.querySelector<HTMLInputElement>('[data-joint-step]')?.value);
      const targetInput = Array.from(
        body.querySelectorAll<HTMLInputElement>('input[type="number"][data-joint-target]'),
  ...
  body.querySelector<HTMLButtonElement>('[data-joint-home]')?.addEventListener('click', () => {
...
function renderDroneControl(
  ...
  const articulations = articulationsForActor(actor, scene);
  const actuatorStates = new Map(
    (simulationStore.current.simulationState?.actuators ?? []).map((item) => [item.id, item]),
  );
  ...
  for (const input of body.querySelectorAll<HTMLInputElement>('[data-rotor-control]')) {
    input.addEventListener('change', () => {
      const actuatorId = input.dataset.rotorControl;
      if (actuatorId) void sendActuatorControls({ [actuatorId]: Number(input.value) });
    });
  }
  body.querySelector<HTMLButtonElement>('[data-rotor-stop]')?.addEventListener('click', () => {
...
function renderSensorView(
  ...
  const sensorArticulation = articulations.find(
    (item) => item.sensors.some((sensor) => sensor.id === selectedSensor.id),
  );
  ...
  const field = (label: string, value: string, name: string): string =>
    `<div class="property-row"><label>${label}</label><input type="text" value="${escapeHtml(value)}" disabled data-sensor-field="${name}" data-runtime-sensor-id="${escapeHtml(selectedSensor.id)}"></div>`;
  let payloadFields = '';
...
function updateRuntimeInspector(simulationState: SimulationState | null): void {
  ...
  const controller = document.querySelector<HTMLElement>('[data-controller-status]');
  if (controller) {
  ...
  for (const joint of simulationState.joints) {
    for (const item of document.querySelectorAll<HTMLElement>('[data-joint-position]')) {
      if (item.dataset.jointPosition === joint.id) item.textContent = `${joint.qpos.toFixed(3)} rad`;
    }
    for (const item of document.querySelectorAll<HTMLElement>('[data-joint-qpos]')) {
      if (item.dataset.jointQpos === joint.id) item.textContent = `qpos ${joint.qpos.toFixed(3)}`;
    }
    for (const item of document.querySelectorAll<HTMLElement>('[data-joint-qvel]')) {
      if (item.dataset.jointQvel === joint.id) item.textContent = `qvel ${joint.qvel.toFixed(3)}`;
    }
    for (const input of document.querySelectorAll<HTMLInputElement>('[data-joint-position-field]')) {
  ...
  for (const actuator of simulationState.actuators) {
    for (const input of document.querySelectorAll<HTMLInputElement>('[data-actuator-id]')) {
  ...
  for (const sensor of simulationState.sensors) {
    for (const input of document.querySelectorAll<HTMLInputElement>('[data-runtime-sensor-id]')) {
      if (input.dataset.runtimeSensorId !== sensor.id) continue;
      const field = input.dataset.sensorField;
      if (field === 'sequence') input.value = String(sensor.sequence);
      else if (field === 'time') input.value = sensor.time.toFixed(3);
...
function renderValidation(issues: ValidationIssue[]): void {
  ...
  const badge = document.querySelector<HTMLElement>('[data-tab-badge="validation"]');
  if (badge) {
...
function renderTrajectoryWindows(
  actor: Actor | undefined,
  scene: Scene,
): void {
  ...
  for (const button of playerBody.querySelectorAll<HTMLButtonElement>('[data-trajectory-command]')) {
    button.addEventListener('click', () => {
...
function updateTrajectoryRuntime(simulationState: SimulationState | null): void {
  ...
  const progress = document.querySelector<HTMLProgressElement>('[data-trajectory-progress]');
  if (progress) {
    progress.max = Math.max(duration, 0.001);
    progress.value = Math.min(time, progress.max);
  }
  const timeLabel = document.querySelector<HTMLElement>('[data-trajectory-time]');
  ...
  const pause = document.querySelector<HTMLButtonElement>('[data-trajectory-command="pause"]');
  const stop = document.querySelector<HTMLButtonElement>('[data-trajectory-command="stop"]');
...
function updateRecordingRuntime(simulationState: SimulationState | null): void {
  ...
  const start = document.querySelector<HTMLButtonElement>('[data-recording-command="start"]');
  const stop = document.querySelector<HTMLButtonElement>('[data-recording-command="stop"]');
  if (start) start.disabled = active;
  if (stop) stop.disabled = !active;
  for (const button of document.querySelectorAll<HTMLButtonElement>('[data-recording-export]')) {
...
function renderControllerPanel(
  ...
  for (const button of controls.querySelectorAll<HTMLButtonElement>('[data-controller-command]')) {
    button.addEventListener('click', () => {
...
function updateControllerRuntime(simulationState: SimulationState | null): void {
  ...
  const name = document.querySelector<HTMLElement>('[data-controller-name]');
  if (name && attached && controller.name) name.textContent = controller.name;
  const steps = document.querySelector<HTMLElement>('[data-controller-steps]');
  if (steps) steps.textContent = `${controller?.step_count ?? 0} Steps`;
  const duration = document.querySelector<HTMLElement>('[data-controller-duration]');
  if (duration) {
  ...
  const detach = document.querySelector<HTMLButtonElement>('[data-controller-command="detach"]');
...
function updateSimulationClock(simulationState: SimulationState | null): void {
  ...
  for (const button of document.querySelectorAll<HTMLButtonElement>('[data-panel-speed]')) {
  ...
  if (panelReadout) panelReadout.textContent = `${actual.toFixed(2)}x`;
  const panelSimRtf = document.getElementById('panel-sim-rtf');
  if (panelSimRtf) panelSimRtf.textContent = `${actual.toFixed(2)}x`;
  const panelSimTime = document.getElementById('panel-sim-time');
  if (panelSimTime) panelSimTime.textContent = `${(simulationState?.time ?? 0).toFixed(2)}s`;
  const targetReadout = document.getElementById('panel-target-speed');
  if (targetReadout) targetReadout.textContent = `${targetRealtimeFactor}x`;
  ...
  const ratio = running && targetRealtimeFactor > 0
    ...
    : 0;
  const meter = document.querySelector<HTMLElement>('.throughput-meter');
  ...
  if (meterFill) meterFill.style.width = `${ratio * 100}%`;
...
function render(): void {
  ...
  if (projectPanelName) projectPanelName.textContent = state.scene.name;
  ...
  if (projectActorCount) projectActorCount.textContent = String(actorCount);
  const projectRobotCount = document.getElementById('project-robot-count');
  if (projectRobotCount) projectRobotCount.textContent = String(articulationCount);
  ...
  const runToggle = element<HTMLButtonElement>('run-toggle-button');
  ...
  const playIcon = runToggle.querySelector<SVGElement>('.run-icon-play');
  const pauseIcon = runToggle.querySelector<SVGElement>('.run-icon-pause');
  if (playIcon) playIcon.toggleAttribute('hidden', running);
  if (pauseIcon) pauseIcon.toggleAttribute('hidden', !running);
  element<HTMLButtonElement>('sim-stop-button').disabled = simulation.simulationStatus === 'stopped';
  const panelRunToggle = element<HTMLButtonElement>('panel-run-toggle');
  panelRunToggle.dataset.menuCommand = running ? 'pause' : 'run';
  const panelRunLabel = running
    ? 'Pause'
    : simulation.simulationStatus === 'paused' ? 'Resume' : 'Run';
  panelRunToggle.textContent = panelRunLabel;
  panelRunToggle.setAttribute(
    'aria-label',
    `${panelRunLabel} simulation from panel`,
  );
  panelRunToggle.classList.toggle('primary', !running);
  const panelStop = document.querySelector<HTMLButtonElement>(
    '[data-panel="sim-control"] [data-menu-command="stop"]',
  );
  if (panelStop) panelStop.disabled = simulation.simulationStatus === 'stopped';
...
function updateStatusBar(): void {
  ...
  if (dot) {
    dot.className = `status-dot ${
      simulation.simulationStatus === 'running' ? 'ok'
  ...
  if (recEl) {
    ...
    if (recording?.active) recEl.textContent = `● rec ${(recording.sample_count ?? 0).toLocaleString()} rows`;
  }
  ...
  if (errorsEl) {
    ...
    if (errors > 0) errorsEl.textContent = `${errors} preflight error(s)`;
  }
...
element<HTMLInputElement>('asset-filter-input').addEventListener('input', (event) => {
  assetFilter = (event.currentTarget as HTMLInputElement).value;
  renderAssets(store.current.assets);
});

for (const button of document.querySelectorAll<HTMLButtonElement>('[data-material-preset]')) {
  button.addEventListener('click', () => {
    ...
    const physics: PhysicsProperties = structuredClone(
      actor.properties.physics ?? { dynamic: true },
    );
    physics.material = preset as PhysicsProperties['material'];
...
function renderWindowMenu(): void {
  ...
  for (const root of document.querySelectorAll<HTMLElement>('[data-panel]')) {
    const id = (root.dataset.panel ?? '') as Parameters<typeof workspace.isOpen>[0];
...
function closeAllMenus(): void {
  for (const dropdown of document.querySelectorAll<HTMLElement>('.menu-dropdown')) {
    dropdown.hidden = true;
  }
  for (const trigger of document.querySelectorAll<HTMLButtonElement>('.menu-trigger')) {
...
for (const menu of document.querySelectorAll<HTMLElement>('.menu')) {
  ...
  trigger?.addEventListener('click', (event) => {
    ...
    closeAllMenus();
    if (dropdown && opening) {
      if (dropdown.contains(windowMenuItems)) renderWindowMenu();
...
document.addEventListener('click', (event) => {
  if (!(event.target as HTMLElement).closest('.menu')) closeAllMenus();
});

document.addEventListener('click', (event) => {
  const target = event.target as HTMLElement;
  const commandItem = target.closest<HTMLElement>('[data-menu-command]');
  if (commandItem) {
    closeAllMenus();
    void handleCommand(commandItem.dataset.menuCommand ?? '');
    return;
  }
  const actionItem = target.closest<HTMLElement>('[data-menu-action]');
  ...
  if (action === 'reset-layout') {
    ...
    if (workspace.applyPreset(presetId)) {
      (element('layout-preset') as HTMLSelectElement).value = presetId;
      renderWindowMenu();
      showToast(`Layout preset applied: ${presetId}`);
    }
  } else if (action === 'open-shortcuts') workspace.openPanel('shortcuts');
  else if (action === 'open-library') workspace.openLibrary();
});
...
for (const button of document.querySelectorAll<HTMLButtonElement>('[data-viewport-option]')) {
  button.addEventListener('click', () => {
    const option = button.dataset.viewportOption as 'grid' | 'fog';
...
async function initialize(): Promise<void> {
  ...
  if (backendStatus) backendStatus.textContent = 'backend connected';

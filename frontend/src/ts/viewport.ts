...
function addStreamedSceneActor(
  ...
  void localSceneManifestResolver(sceneId).then(async (manifest) => {
    ...
    let loaded = 0;
    let failed = 0;
    const resolveChunk = async (chunkId: string): Promise<ArrayBuffer | null> => {
      for (let attempt = 0; attempt < 3; attempt += 1) {
        const buffer = await localSceneChunkResolver(sceneId, chunkId);
        if (buffer) return buffer;
        if (!actorLoadIsCurrent(actor.id, root, loadRevision)) return null;
        if (attempt < 2) {
    ...
    const loadNext = async (): Promise<void> => {
      while (cursor < chunks.length) {
        ...
        if (!actorLoadIsCurrent(actor.id, root, loadRevision)) return;
        if (!buffer) {
          failed += 1;

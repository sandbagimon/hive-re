...
try:
    ...
    raise RuntimeError(
        "gRPC support is not installed. Install SimLab with: pip install -e '.[remote]'"
    ) from exc
...
def serve_grpc_backend(
    ...
    *,
    ...
    server.start()
    print(f"SimLab algorithm gRPC server listening on {bind} (port {port})")
    try:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the SimLab algorithm gRPC backend")
    parser.add_argument("--bind", default="127.0.0.1:50051")
    parser.add_argument("--token", default=os.environ.get("SIMLAB_ALGORITHM_TOKEN"))

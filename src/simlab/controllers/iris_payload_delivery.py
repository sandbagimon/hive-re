...
class IrisPayloadDeliveryController:
    ...
    def _advance_mission(
        ...
        if self.phase == "spool" and elapsed >= self.segment_duration:
            self._start_segment(
                "takeoff", (-2.0, 0.0, CRUISE_HEIGHT), 2.5, observation
            )
        elif self.phase == "takeoff" and segment_reached:
            self._start_segment(
                "to_pickup", (*PICKUP, CRUISE_HEIGHT), 3.0, observation
            )
        elif self.phase == "to_pickup" and segment_reached:
            self._start_segment(
                "descend_pickup", (*PICKUP, HOOK_HEIGHT), 3.0, observation
            )
            ...
            self._start_segment(
                "lift_payload", (*PICKUP, CRUISE_HEIGHT), 3.0, observation
            )
        elif self.phase == "lift_payload" and segment_reached:
            self._start_segment(
                "to_dropoff", (*DROPOFF, CRUISE_HEIGHT), 5.0, observation
            )
        elif self.phase == "to_dropoff" and segment_reached:
            self._start_segment(
                "descend_dropoff", (*DROPOFF, HOOK_HEIGHT), 3.0, observation
            )
            ...
            self._start_segment(
                "retreat", (*DROPOFF, CRUISE_HEIGHT), 3.0, observation
            )

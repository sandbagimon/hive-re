...
class SimLabApiError(RuntimeError):
    """A transport or application error returned by the SimLab REST API."""


class SimLabApiClient:
    ...
    def _resource_id(value: str, prefix: str) -> str:
        if not re.fullmatch(rf"{prefix}_[A-Za-z0-9_-]+", value):
            raise SimLabApiError(f"Invalid SimLab {prefix} resource ID")
        return value
    ...
    async def _request(
        ...
        *,
        ...
        try:
            ...
            raise SimLabApiError(f"SimLab API unavailable: {error}") from error
        if response.is_error:
            ...
            raise SimLabApiError(
                f"SimLab API {method} {response.url.path} failed "
        ...
        try:
            ...
            raise SimLabApiError(
                f"SimLab API returned non-JSON data for {method} {response.url.path}"
            ) from error
        if not isinstance(payload, dict):
            raise SimLabApiError(
                f"SimLab API returned an invalid object for {method} {response.url.path}"
            )
        return payload
    ...
    async def simulation_command(
        ...
        if command not in {"run", "pause", "step", "reset"}:
            raise SimLabApiError("Invalid SimLab simulation command")

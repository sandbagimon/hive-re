const cloneTargets = (targets) => ({ ...targets });
const sortKeyframes = (keyframes) => [...keyframes].sort((left, right) => left.time - right.time || left.id.localeCompare(right.id));
export function createTrajectoryDraft(actorId, homeTargets, duration = 2) {
...
export function setTrajectoryDuration(draft, duration) {
    if (!Number.isFinite(duration) || duration <= 0) {
        throw new Error('Trajectory duration must be greater than zero');
    }
...
export function captureTrajectoryKeyframe(draft, id, time, targets) {
    return {
        ...draft,
...
export function updateTrajectoryKeyframeTime(draft, id, time) {
    return {
        ...draft,
...
export function updateTrajectoryKeyframeTarget(draft, id, jointId, target) {
    return {
        ...draft,
...
export function removeTrajectoryKeyframe(draft, id) {
...
export function validateTrajectoryDraft(draft) {
    const issues = [];
    if (!draft.name.trim())
        issues.push('Trajectory name cannot be empty');
    if (draft.keyframes.length < 2) {
    ...
    draft.keyframes.forEach((keyframe, index) => {
        ...
        for (const [jointId, target] of Object.entries(keyframe.targets)) {
            if (!jointId)
                issues.push(`Keyframe ${index} contains an empty joint ID`);
...
export function trajectoryFromDraft(draft) {

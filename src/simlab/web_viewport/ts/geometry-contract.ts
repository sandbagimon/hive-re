import type { Actor, PrimitiveType } from './types.js';

export interface GeometryDescription {
  geomType: PrimitiveType;
  size: number[];
}

const aliases: Record<string, PrimitiveType> = {
  primitive_box: 'box',
...
const defaults: Record<PrimitiveType, number[]> = {
  box: [0.5, 0.5, 0.5],
  sphere: [0.5],
  cylinder: [0.5, 0.5],
...
export function sourceGeometry(actor: Actor): GeometryDescription {
...
export function colliderGeometry(actor: Actor): GeometryDescription {

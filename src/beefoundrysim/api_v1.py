...
from beefoundrysim.services.openusd.upload_bundle import MAX_UPLOAD_BYTES, MAX_UPLOAD_FILES
...
def create_v1_router(manager: ResourceManager, access_token: str | None) -> APIRouter:
    ...
    async def local_scene_chunk(
        ...
        return StreamingResponse(
            content(),
            media_type="application/vnd.beefoundrysim.geometry-bundle",
            headers=headers,
        )
    ...
    async def download_artifact(artifact_id: str, request: Request) -> Response:
        ...
        headers = {
            ...
            "Content-Encoding": "identity",
            "X-BeeFoundrySim-Artifact-Id": artifact.id,

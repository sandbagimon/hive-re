...
from beefoundrysim.mcp.api_client import BeeFoundrySimApiClient
...
def create_mcp_server(api: BeeFoundrySimApiClient) -> MCPServer:
    """Create an MCP server backed exclusively by the versioned BeeFoundrySim REST API."""
    ...
    server = MCPServer(
        "beefoundrysim",
        title="BeeFoundrySim Robotics Simulation",
        version="0.1.0",
        instructions=(
            "Inspect a project and run beefoundrysim_preflight_project before exporting or "
    ...
    async def beefoundrysim_health() -> dict[str, Any]:
        """Check whether the configured BeeFoundrySim REST backend is reachable."""
        return await api.health()

    @server.tool(annotations=MUTATING, structured_output=True)
    async def beefoundrysim_create_project(name: str = "Untitled Project") -> dict[str, Any]:
        """Create an isolated BeeFoundrySim project and return its opaque project ID."""
        return await api.create_project(name)

    @server.tool(annotations=READ_ONLY, structured_output=True)
    async def beefoundrysim_get_project(project_id: str) -> dict[str, Any]:
    ...
    async def beefoundrysim_update_scene(
    ...
    @server.tool(annotations=READ_ONLY, structured_output=True)
    async def beefoundrysim_list_assets(project_id: str) -> dict[str, Any]:
        """List assets available inside one BeeFoundrySim project's asset catalog."""
        return await api.list_assets(project_id)

    @server.tool(annotations=READ_ONLY, structured_output=True)
    async def beefoundrysim_preflight_project(project_id: str) -> dict[str, Any]:
        """Validate physics and ask MuJoCo to compile the generated model without running it."""
        return await api.preflight(project_id)

    @server.tool(annotations=MUTATING, structured_output=True)
    async def beefoundrysim_export_mjcf(project_id: str) -> dict[str, Any]:
        """Create an MJCF artifact after project preflight and return its download metadata."""
        return await api.export_mjcf(project_id)

    @server.tool(annotations=MUTATING, structured_output=True)
    async def beefoundrysim_create_simulation(project_id: str) -> dict[str, Any]:
    ...
    @server.tool(annotations=READ_ONLY, structured_output=True)
    async def beefoundrysim_get_simulation_snapshot(simulation_id: str) -> dict[str, Any]:
        """Read the current state, status, sensors, and sequence of a simulation."""
        return await api.simulation_snapshot(simulation_id)

    @server.tool(annotations=MUTATING, structured_output=True)
    async def beefoundrysim_run_simulation(simulation_id: str) -> dict[str, Any]:
    ...
    async def beefoundrysim_stop_simulation(simulation_id: str) -> dict[str, Any]:
        """Stop and permanently delete a simulation resource."""
        return await api.stop_simulation(simulation_id)

    @server.tool(annotations=IDEMPOTENT_WRITE, structured_output=True)
    async def beefoundrysim_set_simulation_speed(
    ...
    @server.tool(annotations=IDEMPOTENT_WRITE, structured_output=True)
    async def beefoundrysim_set_joint_targets(
    ...
    @server.tool(annotations=IDEMPOTENT_WRITE, structured_output=True)
    async def beefoundrysim_set_actuator_controls(
    ...
    @server.resource(
        ...
        name="beefoundrysim-health",
        description="Current BeeFoundrySim API health status.",
    ...
    @server.resource(
        ...
        name="beefoundrysim-project",
        description="Canonical BeeFoundrySim project scene and revision.",
        mime_type="application/json",
    )
    ...
    @server.resource(
        ...
        name="beefoundrysim-project-assets",
        description="Project-scoped BeeFoundrySim asset catalog.",
        mime_type="application/json",
    )
    ...
    @server.resource(
        ...
        name="beefoundrysim-simulation-snapshot",
        description="Current BeeFoundrySim simulation snapshot.",
        mime_type="application/json",
    )
    ...
    @server.prompt()
    def beefoundrysim_review_project(project_id: str) -> str:
        """Build a safe project inspection and simulation-readiness workflow."""
        return (
            f"Review BeeFoundrySim project {project_id}. Read beefoundrysim://projects/{project_id}, "
            f"read beefoundrysim://projects/{project_id}/assets, then call "
            f"beefoundrysim_preflight_project with project_id={project_id}. Summarize blocking "
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Expose the BeeFoundrySim REST API through Model Context Protocol."
    )
    parser.add_argument(
        "--api-url",
        default=os.environ.get("BEEFOUNDRYSIM_API_URL", "http://127.0.0.1:8765"),
        help="BeeFoundrySim API origin or /api/v1 URL (default: %(default)s)",
    )
    parser.add_argument(
        "--transport",
        choices=("stdio", "streamable-http"),
        default=os.environ.get("BEEFOUNDRYSIM_MCP_TRANSPORT", "stdio"),
    )
    parser.add_argument(
        "--host", default=os.environ.get("BEEFOUNDRYSIM_MCP_HOST", "127.0.0.1")
    )
    parser.add_argument(
        "--port", type=int, default=int(os.environ.get("BEEFOUNDRYSIM_MCP_PORT", "8766"))
    )
    parser.add_argument(
        "--path", default=os.environ.get("BEEFOUNDRYSIM_MCP_PATH", "/mcp")
    )
    parser.add_argument(
        "--allow-remote",
        action="store_true",
    ...
    parser.add_argument(
        "--timeout",
        type=float,
        default=float(os.environ.get("BEEFOUNDRYSIM_MCP_API_TIMEOUT", "30")),
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    api = BeeFoundrySimApiClient(
        args.api_url,
        access_token=os.environ.get("BEEFOUNDRYSIM_API_TOKEN"),

...
class LiveOccupancyGrid:
    ...
    def add_transient_obstacle(self, point: Point2D, now: float) -> bool:
        """Mark one externally observed obstacle cell, e.g. a neighbouring vehicle.

        The cell follows the same TTL expiry as rangefinder hits, so a moving
        intruder frees its cells once it stops being re-reported.
        """
        if not math.isfinite(now) or now < 0:
            raise ValueError("Observation time must be finite and >= 0")
        cell = self.spec.cell(point, clamp=True)
        if not self.spec.contains(cell) or cell in self.static_cells:
            return False
        if self._observed_at.get(cell) == now:
            return False

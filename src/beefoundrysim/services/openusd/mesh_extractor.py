...
def mesh_to_obj(positions: list[float], indices: list[int]) -> str:
    """Serialize triangle mesh arrays to a MuJoCo-compatible OBJ document."""
    lines = ["# Generated by BeeFoundrySim OpenUSD importer"]
    ...
    for index in range(0, len(indices), 3):
        lines.append(
            f"f {indices[index] + 1} {indices[index + 1] + 1} {indices[index + 2] + 1}"
        )
    return "\n".join(lines) + "\n"
...
def extract_prim_mesh(
    prim: Any,
    *,
    up_axis: str,
    meters_per_unit: float,
    local_scale: list[float] | tuple[float, float, float] = (1.0, 1.0, 1.0),
) -> MeshData:
    """Extract one USD geometry prim in its local frame for viewport caching."""
    try:
    ...
    positions, indices, uvs, _native = _local_geometry(prim, UsdGeom)
    if not positions or not indices:
        return MeshData(source_prim_paths=[str(prim.GetPath())])
    positions = [
...
def _geometry_instances(
    ...
    *,
    expand_point_instancers: bool = True,
    ...
    if not expand_point_instancers:
        return output, 0

    copies = _PointInstancerCopies(
        stage, xform_cache, usd, usd_geom, warnings, path_filter
    )
    for _root_path, prim, matrix, visible in copies:
        output.append(
            _GeometryInstance(
                prim=prim,
                matrix=matrix,
...
class _PointInstancerCopies:
    """Iterate PointInstancer copies with nested instancers composed.

    PointInstancers nested inside another instancer's prototype subtree are
    expanded through their parent's instance transforms instead of their
    authored stage location, so prototype-authored scatter content lands on
    every painted instance rather than collapsing onto the prototype's own
    position. Each iteration yields
    ``(root_instancer_path, prim, composed_world_matrix, visible)`` and
    ``instance_count`` reports the preserved instances of every expanded
    instancer once iteration completes.
    """
    ...
    def __init__(
        ...
        self._instancer_cache: dict[str, tuple[list[Any], list[int], list[Any]]] = {}
        self._prototype_cache: dict[
            str, tuple[list[tuple[Any, Any]], list[tuple[Any, Any]]]
        ] = {}
        self._counted_instancers: set[str] = set()
        self.instance_count = 0
        instancers = [
            prim for prim in stage.Traverse() if prim.IsA(usd_geom.PointInstancer)
        ]
        prototype_roots = {
            str(path)
            for prim in instancers
            for path in usd_geom.PointInstancer(prim).GetPrototypesRel().GetTargets()
        }
        self._root_instancers = [
            prim
            for prim in instancers
            if not _inside_prototype(str(prim.GetPath()), prototype_roots)
        ]

    def __iter__(self) -> Iterator[tuple[str, Any, Any, bool]]:
        for prim in self._root_instancers:
    ...
    def _expand(
        ...
        instancer_path = str(instancer_prim.GetPath())
        if self._path_filter is not None and not self._path_filter(instancer_path):
            return
        chain_visible = chain_visible and _is_visible(instancer_prim, self._usd_geom)
        prototypes, proto_indices, transforms = self._instancer_data(instancer_prim)
        if instancer_path not in self._counted_instancers:
            self._counted_instancers.add(instancer_path)
        ...
        for instance_index, instance_matrix in enumerate(transforms):
            if instance_index >= len(proto_indices):
                self._warnings.append(
                    f"PointInstancer {instancer_prim.GetPath()} has no prototype index "
                    f"for instance {instance_index}."
                )
                continue
            prototype_index = int(proto_indices[instance_index])
            if prototype_index < 0 or prototype_index >= len(prototypes):
                self._warnings.append(
                    f"PointInstancer {instancer_prim.GetPath()} references invalid "
                    f"prototype index {prototype_index}."
                )
                continue
            prototype = self._stage.GetPrimAtPath(prototypes[prototype_index])
            if not prototype.IsValid():
                continue
            subtree_base = instance_matrix * base
            parts, nested = self._prototype_parts(prototype)
            for prim, relative in parts:
                yield (
                    root_path,
                    prim,
                    relative * subtree_base,
                    chain_visible and _is_visible(prim, self._usd_geom),
                )
            for prim, relative in nested:
                nested_path = str(prim.GetPath())
                if nested_path in ancestors:
                    self._warnings.append(
                        f"Skipped cyclic PointInstancer nesting at {nested_path}."
                    )
                    continue
                yield from self._expand(
                    prim,
                    ...
                    root_path,
                    chain_visible,
                    ancestors | {nested_path},
                )

    def _instancer_data(self, prim: Any) -> tuple[list[Any], list[int], list[Any]]:
        path = str(prim.GetPath())
        cached = self._instancer_cache.get(path)
        if cached is None:
            instancer = self._usd_geom.PointInstancer(prim)
            cached = (
                list(instancer.GetPrototypesRel().GetTargets()),
                list(instancer.GetProtoIndicesAttr().Get() or []),
                list(
            ...
            self._instancer_cache[path] = cached
        return cached

    def _prototype_parts(
        self, prototype: Any
    ) -> tuple[list[tuple[Any, Any]], list[tuple[Any, Any]]]:
        """Split a prototype subtree into geometry parts and nested instancers.

        Each entry carries the prim's transform relative to the prototype root;
        descendants of nested PointInstancers are pruned from the direct parts
        so they are only emitted through the nested expansion.
        """
        path = str(prototype.GetPath())
        cached = self._prototype_cache.get(path)
        if cached is not None:
            return cached
        parts: list[tuple[Any, Any]] = []
        nested: list[tuple[Any, Any]] = []
        nested_roots: list[str] = []
        prototype_world = self._xform_cache.GetLocalToWorldTransform(prototype)
        for prim in self._usd.PrimRange(prototype):
            prim_path = str(prim.GetPath())
            if any(prim_path.startswith(f"{root}/") for root in nested_roots):
                continue
            if prim.IsA(self._usd_geom.PointInstancer):
                nested_roots.append(prim_path)
                if (
                    self._path_filter is not None
                    and not self._path_filter(prim_path)
                ) or not _is_visible(prim, self._usd_geom):
                    continue
                nested.append(
                    (
                        prim,
                ...
                continue
            if not _is_supported_geometry(prim, self._usd_geom):
                continue
            if self._path_filter is not None and not self._path_filter(prim_path):
                continue
            parts.append(
                (
                    prim,
        ...
        cached = (parts, nested)
        self._prototype_cache[path] = cached
        return cached
    ...
    try:
        ...
        if material and material.GetPrim().IsValid():
            ...
            if not shader or not shader.GetPrim().IsValid():
                mdl_source = material.ComputeSurfaceSource("mdl")
                shader = mdl_source[0] if mdl_source else None

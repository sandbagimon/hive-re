...
from beefoundrysim.services.local_scene_streaming import LocalSceneService
...
class ResourceManager:
    ...
    def _rebind_catalog_references(
        ...
        for local_asset in self.local_scenes.assets(project.root):
    ...
    def assets(self, project_id: str) -> list[dict[str, Any]]:
        ...
        assets.extend(
            self.externalize(project, local_asset)
            for local_asset in self.local_scenes.assets(project.root)
        )
        return assets

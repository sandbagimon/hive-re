...
from beefoundrysim.simulation.task import BoundEnvironmentTask, EnvironmentTask
...
class BeeFoundrySimEnv(_GymBase):  # type: ignore[misc,valid-type]
    ...
    def __init__(
        ...
        *,
        ...
        if gym is None or spaces is None:
            raise RuntimeError(
                "Gymnasium is not installed. Install BeeFoundrySim with: pip install -e '.[algorithm]'"
            )
        ...
        if render_mode not in {None, "state"}:
            raise ValueError("BeeFoundrySimEnv render_mode must be None or 'state'")
    ...
    def _require_session(self) -> SimulationBackendSession:
        if self._session is None:
            raise RuntimeError("BeeFoundrySimEnv is closed")
